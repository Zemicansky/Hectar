...
function paint3DSoilCoverage(x, z, widthMeters, headingDeg) {
  ...
  if (prevL && prevR) {
    ...
    for (let s = 1; s <= steps; s++) {
      ...
      // FIX v3.0 (перепроверено — мерцание следа во время движения ПО-ПРЕЖНЕМУ
      // было, предыдущий фикс не устранил настоящую причину): дело не в
      // z-fighting между сегментами (они лежат в одной Y-плоскости 0.06 и
      // стыкуются краями без геометрического наложения — тут действительно
      // нечему "бороться" за один и тот же пиксель глубины). Настоящая причина
      // в другом: material.transparent=true заставляет Three.js на КАЖДОМ
      // кадре пересортировывать все прозрачные объекты сцены по расстоянию
      // до камеры перед рендером (это стандартное поведение WebGLRenderer
      // для transparent-объектов, чтобы alpha-blending был визуально корректен
      // при просвечивании друг через друга). У нас тысячи почти-соседних по
      // глубине сегментов ленты, а камера каждый кадр немного двигается вслед
      // за трактором (см. animate(), camera3D.position.lerp) — при таком
      // движении относительный порядок соседних по расстоянию сегментов
      // на грани погрешности "перещёлкивается" от кадра к кадру, и порядок
      // отрисовки/блендинга скачет — визуально это читается как мерцание
      // именно во время движения (камера едет — сортировка "дёргается";
      // трактор стоит — камера тоже стоит — сортировка стабильна, мерцания нет,
      // что совпадает с исходным симптомом "мерцает именно во время заезда").
      // Лента визуально непрозрачна (закрашенный участок почвы, не должна
      // просвечивать землю под собой) — сортировка по прозрачности ей не
      // нужна вообще. Убираем transparent (opacity игнорируется без него,
      // цвет берём чуть темнее, чтобы визуально сохранить прежний "приглушённый"
      // вид без alpha-блендинга) — рендер идёт по обычному Z-буферу без
      // пересортировки, мерцание от нестабильного порядка блендинга уходит.
      // polygonOffset оставлен как страховка от редких копланарных стыков
      // с землёй/сеткой на самой границе сегмента.
